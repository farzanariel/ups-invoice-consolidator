...
export function parseCSV(
  ...
  return new Promise((resolve) => {
    ...
    reader.onload = (e) => {
      ...
      // Prepend the canonical header row so headerless CSVs parse correctly.
      // UPS invoice exports never include a header row by default.
      const csvText = UBD_HEADER_ROW + '\n' + rawText;

      Papa.parse<UPSInvoiceRow>(csvText, {
        header: true,
        skipEmptyLines: true,
        complete: (results) => {
          // TooFewFields / TooManyFields are non-fatal field-count mismatches
          // that occur when the CSV has fewer columns than our header row.
          // Data is still parsed correctly for the columns that are present.
          ...
          if (fatalErrors.length > 0) {
            resolve({
              data: [],
              error: `CSV parsing errors: ${fatalErrors
                .map((e) => e.message)
                .join(', ')}`,
            });
          } else {
